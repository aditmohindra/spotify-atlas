import { TrackPoint, ClusterInfo } from "@/hooks/useMapData";
import { getClusterColor } from "./GalaxyMap";
  ...
  return (
    ...
    >
      ...
      <div className="px-5 py-4 flex flex-col gap-4 flex-1">
        <div>
          <div className="text-white/25 text-xs uppercase tracking-widest mb-2">Community</div>
          <div className="flex items-center gap-2 mb-1">
            <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ background: color, boxShadow: `0 0 6px ${color}` }} />
            <span className="text-white/80 text-xs font-medium">
              {cluster?.name ?? (track.cluster_id === -1 ? "Unclassified" : `Cluster ${track.cluster_id}`)}
            </span>
          </div>
          {cluster?.canonical_name && (
            <p className="text-white/30 text-xs pl-4">{cluster.canonical_name}</p>
          )}
          {cluster?.description && (
            <p className="text-white/40 text-xs mt-2 leading-relaxed">{cluster.description}</p>
          )}
        </div>

        {cluster?.keywords && cluster.keywords.length > 0 && (
          <div>
            <div className="text-white/25 text-xs uppercase tracking-widest mb-2">Keywords</div>
