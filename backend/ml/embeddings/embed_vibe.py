"""
Embed vibe_combined_document for all tracks.
Stores in track_embeddings with document_type='vibe'.
Pushes to Qdrant collection tracks_vibe.

Usage:
    uv run python ml/embeddings/embed_vibe.py
"""
...
async def embed_vibe_documents() -> int:
    db = SessionLocal()
    ...
    try:
        ...
        tracks = (
            db.query(Track)
            .filter(Track.vibe_combined_document.isnot(None))
            .all()
        )

        tracks_to_embed = [t for t in tracks if t.id not in existing_ids]
        print(f"[vibe] {len(tracks_to_embed)} tracks to embed ({len(existing_ids)} already done)")

        total_embedded = 0

        for i in range(0, len(tracks_to_embed), BATCH_SIZE):
            batch = tracks_to_embed[i:i + BATCH_SIZE]
            texts = [t.vibe_combined_document for t in batch]
            ...
            try:
                ...
                print(f"  [vibe] Embedded {total_embedded}/{len(tracks_to_embed)}")

            except Exception as e:
                print(f"  [vibe] Error on batch {i}: {e}")
                db.rollback()
                await asyncio.sleep(5)

            await asyncio.sleep(0.1)

        print(f"[vibe] Done. Total embedded: {total_embedded}")
...
async def main():
    embedded = await embed_vibe_documents()
