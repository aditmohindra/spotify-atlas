...
# Only check MULTI-WORD artist names - single common words cause false positives
# (e.g. an artist literally named "Clouds" will false-match prose that says "clouds")
artist_patterns = {}
...
print("\nLeaking multi-word artist names (by track count):")
top = sorted(leak_counts.items(), key=lambda x: -x[1])[:30]
for aid, cnt in top:
